...
class _SettingsViewState extends State<SettingsView> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
      listenable: Listenable.merge([servers, connection, runner]),
      builder: (context, _) {
                    ...
                    children: [
                      ...
                      SizedBox(
                        ...
                        child: TextField(
                          ...
                          style: const TextStyle(color: depotText, fontWeight: FontWeight.w700),
                          decoration: depotInputDecoration('用户名', hint: 'root', icon: Icons.person_outline),
                        ),
                      ),
                      FilledButton.icon(
                        onPressed: () async {
                  ...
                  if (servers.servers.isEmpty)
                    ...
                    for (final server in servers.servers) ...[
                      _ServerRow(
                        ...
                        onDelete: () async {
                          if (connection.target?.host == server.host && connection.target?.user == server.user) {
